...
class ttt_board { private: char board[3][3]; public: static const char NOBODY = ' ', PLAYER1 = 'X', PLAYER2 = 'O';
...
ttt_board::ttt_board() { init_board(); }

void ttt_board::init_board() { for(int x = 0; x < 3; x ++) { for(int y = 0; y < 3; y ++) { board[x][y] = NOBODY; } } }

void ttt_board::display() { cout << endl;
...
void ttt_board::set(int x, int y, char c) { board[x][y] = c; }

char ttt_board::get(int x, int y) const { return board[x][y]; }

class opponent { private: string name; protected: char c;
void set_name(string name, char c); public: virtual void move(const ttt_board &board, int &xpos, int &ypos) = 0; string get_name() { return name; } };

void opponent::set_name(string name, char c) { this->name = name + " (" + c + ")"; this->c = c; }

class aiopponent : public opponent { private: int two(char c, char x, char y, char z); public: aiopponent(string name, char c); void move(const ttt_board &board, int &xpos, int &ypos); };

aiopponent::aiopponent(string name, char c) { set_name(name, c); }

int aiopponent::two(char c, char x, char y, char z) { if(x == c && y == c && z == ttt_board::NOBODY) return 3; if(x == c && y == ttt_board::NOBODY && z == c) return 2; if(x == ttt_board::NOBODY && y == c && z == c) return 1;

return 0;
}

void aiopponent::move(const ttt_board &board, int &xpos, int &ypos) { int r; char ic; if(c == ttt_board::PLAYER1) ic = ttt_board::PLAYER2; else ic = ttt_board::PLAYER1; for(int x = 0; x < 3; x ++) { if((r = two(c, board.get(0, x), board.get(1, x), board.get(2, x)))) { ypos = x, xpos = r-1; return; }
...
class humanopponent : public opponent { public: humanopponent(char c); void move(const ttt_board &board, int &xpos, int &ypos); };

humanopponent::humanopponent(char c) { cout << "\nEnter your name: "; string name; cin >> name; set_name(name, c); }

void humanopponent::move(const ttt_board &board, int &xpos, int &ypos) { for(;;) { cout << "Enter column (x-axis) number (1-3): "; while(!(cin >> xpos) || xpos <= 0 || xpos > 3) { // !!! cin.clear(); cout << ": "; }
...
class ttt { private: ttt_board board; opponent *player[2]; int turn;

public: ttt(); ~ttt(); bool new_game(); void play_game(); int game_done(); };

ttt::ttt() : turn(0) { player[0] = player[1] = 0; }

ttt::~ttt() { for(int x = 0; x < 2; x ++) { if(player[x]) delete player[x]; } }

bool ttt::new_game() { cout << "\nNew game\nEnter the type of player #1 ([A]I/[H]uman/[Q]uit): ";
...
void ttt::play_game() { int x, y, won; char again;
...
int ttt::game_done() { int n, flag = 0;
